...
var roles =            {harvester:      {namer:"harvester",             minimum:0,      requirement:0,          buildRestriction : false,       run: require('role.harvester')},
			...
			soloraider:     {namer:"soloraider",            minimum:0,      requirement:800,        buildRestriction : false,       run: require('role.raider')},
...
//	profiler.wrap(function() { 
	...
	for (let name in Game.creeps) {
		...
		if(creep.memory.spawnRoom != undefined && Memory.rooms[creep.memory.spawnRoom].role == undefined) {
			...
			if(underAttack[room]) {
				...
				for (let tower of towers) {
					let structure = tower.pos.findClosestByRange(FIND_STRUCTURES, {
							filter: (s) => s.hits != s.hitsMax && s.hits < 1200
					});
