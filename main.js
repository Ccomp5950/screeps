...
var roles =            {harvester:      {namer:"harvester",             minimum:2,      requirement:0,          buildRestriction : false,       run: require('role.harvester')},
			...
			sapper:         {namer:"sapper",                minimum:1,      requirement:-1,         buildRestriction : true,        run: require('role.sapper')},
                        ...
                        miner:          {namer:"miner",                 minimum:1,      requirement:900,        buildRestriction : true,        run: require('role.miner')},
                        fetcher:        {namer:"fetcher",               minimum:1,      requirement:1800,       buildRestriction : true,        run: require('role.fetcher')},
	...
	*/
	...
	if((underAttack[creep.room.name] && !creep.memory.combat) || creep.memory.role == 'towertender') {
		...
		if(roles[creep.memory.role] == null) {
			...
			if((creep.ticksToLive - (creep.body.length * 2) - 5) >= 0) {
				roles[creep.memory.role].current++;
			}
			roles[creep.memory.role].run.run(creep);
		}
