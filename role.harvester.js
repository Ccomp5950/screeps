module.exports = {
	...
	if(creep.approachAssignedFlag(999) == false) {
		...
		if(creep.memory.doNothing >=0) {
			...
			creep.say("Zzzz");
			if(creep.approachAssignedFlag(0) == true && creep.memory.pos != undefined) creep.memory.pos.timer = 0;
		...
		if (structure == null && creep.carry.energy > 0) {
			...
			if(upgraderContainer == undefined) {
				...
				if(flag != undefined) {
					...
					filter:(s) => s.structureType == STRUCTURE_CONTAINER && s.store[RESOURCE_ENERGY] < 1000 && !s.isBeingHandled(creep) && s.id != creep.memory.pulledfrom
					}); 
